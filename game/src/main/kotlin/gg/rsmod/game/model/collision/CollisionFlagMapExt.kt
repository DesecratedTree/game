...
private fun CollisionFlagMap.changeNormalCollision(definitions: DefinitionSet, obj: GameObject, add: Boolean) {
    ...
    val interactType = def.interactive
    val blockProjectile = def.impenetrable
    val breakRouteFinding = def.obstructive

    when {
        shape in GameObjectShape.WALL_SHAPES && interactType -> {
            changeWallCollision(
                location,
                rotation,
                shape,
        ...
        shape in GameObjectShape.NORMAL_SHAPES && interactType -> {
            var width = def.width
        ...
        shape in GameObjectShape.GROUND_DECOR_SHAPES && interactType -> changeFloorDecor(location, add)
    }
}
