...
object WeavingAction {
    ...
    suspend fun weave(task: QueueTask, data: WeavingData, amount: Int) {
    ...
    suspend fun weaveMilestoneCape(task: QueueTask, item: Int, amount: Int) {
        val player = task.player
        val inventory = player.inventory
        val name = player.world.definitions.get(ItemDef::class.java, item).name
    ...
    private suspend fun canWeaveCape(task: QueueTask, item: Int) : Boolean {
        val player = task.player
        val inventory = player.inventory
        val resultName = player.world.definitions.get(ItemDef::class.java, item).name
    ...
    fun getBallsAmount(name: String): Int {
        return name.takeLast(4).replace("(", "").replace(")", "").toInt() / 10
    }

    private fun getMinimumLevelForCape(name: String): Int {
        return name.takeLast(4).replace("(", "").replace(")", "").toInt()
    }
